...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(BlazecoinUnits::BLZ)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(foreground);
        QString amountText = BlazecoinUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(qint64 balance, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    ...
    ui->labelUnconfirmed->setText(BlazecoinUnits::formatWithUnit(unit, unconfirmedBalance));
    ui->labelImmature->setText(BlazecoinUnits::formatWithUnit(unit, immatureBalance));
...
//        ui->listTransactions->setModelColumn(TransactionTableModel::ToAddress);
    ...
    // update the display unit, to not use the default ("BLZ")
    updateDisplayUnit();
}
