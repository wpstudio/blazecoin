    ...
    prevBlocks(0)
{
    ...
    if (!fIsTestnet)
    {
        setWindowTitle(tr("Blazecoin") + " - " + tr("Wallet"));
        QApplication::setWindowIcon(QIcon(":icons/blazecoin"));
        setWindowIcon(QIcon(":icons/blazecoin"));
    }
    else
    {
        setWindowTitle(tr("Blazecoin") + " - " + tr("Wallet") + " " + tr("[testnet]"));
        QApplication::setWindowIcon(QIcon(":icons/blazecoin_testnet"));
        setWindowIcon(QIcon(":icons/blazecoin_testnet"));
    }
#else
    ...
    if (!fIsTestnet)
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/blazecoin"));
    else
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/blazecoin_testnet"));
#endif
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BlazecoinGUI::RunMiningAsStartup()
{
...
void BlazecoinGUI::SetMiningStatus(bool isMining)
{
    //if(ui->checkBox)
       // ui->checkBox->setChecked(isMining);
}

BlazecoinGUI::~BlazecoinGUI()
{
    //saveWindowGeometry();
...
void BlazecoinGUI::createActions(bool fIsTestnet)
{
  ...
  //  connect(ui->bNewMessage, SIGNAL(clicked()), this, SLOT(showNormalIfMinimized()));
    ...
    if (!fIsTestnet)
        aboutAction = new QAction(QIcon(":/icons/blazecoin"), tr("&About Blazecoin"), this);
    else
        aboutAction = new QAction(QIcon(":/icons/blazecoin_testnet"), tr("&About Blazecoin"), this);
    aboutAction->setStatusTip(tr("Show information about Blazecoin"));
    ...
    if (!fIsTestnet)
        toggleHideAction = new QAction(QIcon(":/icons/blazecoin"), tr("&Show / Hide"), this);
    else
        toggleHideAction = new QAction(QIcon(":/icons/blazecoin_testnet"), tr("&Show / Hide"), this);
...
void BlazecoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void BlazecoinGUI::createToolBars()
{
...
void BlazecoinGUI::setClientModel(ClientModel *clientModel)
{
...
void BlazecoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void BlazecoinGUI::createTrayIcon()
{
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);

    trayIcon->setToolTip(tr("Blazecoin client"));
...
void BlazecoinGUI::createTrayIconMenu()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BlazecoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BlazecoinGUI::saveWindowGeometry()
{
    QSettings settings;
...
void BlazecoinGUI::restoreWindowGeometry()
{
    QSettings settings;
...
void BlazecoinGUI::optionsClicked()
{
...
void BlazecoinGUI::aboutClicked()
{
...
void BlazecoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
...
void BlazecoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void BlazecoinGUI::changeEvent(QEvent *e)
{
...
void BlazecoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void BlazecoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
...
void BlazecoinGUI::incomingTransaction(const QModelIndex& parent, int start, int /*end*/)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(BlazecoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}

void BlazecoinGUI::gotoOverviewPage()
{
...
void BlazecoinGUI::gotoHistoryPage()
{
...
void BlazecoinGUI::gotoAddressBookPage()
{
...
void BlazecoinGUI::gotoReceiveCoinsPage()
{
...
//void BlazecoinGUI::gotoSendCoinsPage(QString addr)
void BlazecoinGUI::gotoSendCoinsPage()
{
...
void BlazecoinGUI::gotoSignMessageTab(QString addr)
{
...
void BlazecoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void BlazecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BlazecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
	    ...
	    gotoSendCoinsPage();
            ...
            message(tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Blazecoin address or malformed URI parameters."),
...
bool BlazecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
void BlazecoinGUI::resizeEvent(QResizeEvent *e)
{
    updateMask();
    QMainWindow::resizeEvent(e);
}

void BlazecoinGUI::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
...
void BlazecoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void BlazecoinGUI::menuFileRequested()
{
    QMenu menu(this);
...
void BlazecoinGUI::menuOperationsRequested()
{
    QMenu menu(this);
    QAction* send = menu.addAction(tr("Send Blazecoins"));
    QAction* receive = menu.addAction(tr("Receive Blazecoins"));
...
void BlazecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BlazecoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void BlazecoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void BlazecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void BlazecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BlazecoinGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void BlazecoinGUI::onMiningClicked()
{
