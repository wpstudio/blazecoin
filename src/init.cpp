...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/blazecoin.conf are parsed in qt/blazecoin.cpp's main()
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 55413 or testnet: 75413)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Blazecoin is probably already running."), strDataDir.c_str()));
    ...
    if (pwalletMain)
        GenerateBlazecoins(GetBoolArg("-gen", false), pwalletMain);
